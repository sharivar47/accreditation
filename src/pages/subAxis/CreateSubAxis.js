...
const CreateAxis = () => {
    ...
    const save = async (model, {resetForm}) => {
        const result = await Api.post(ApiUrls.createSubAxis(), model)
        if (result.success) {
            resetForm()
        ...
        <Card>
            <CardHeader>
                <div className="d-flex justify-content-between card-header-custom bg-primary text-white"><span
                    className="text-header">{Str.createSubAxis}</span>
                            ...
                            return (<form className="form-wrapper" style={{paddingTop: "60px"}} onSubmit={(e) => {
                                ...
                                <div className="row mb-4">
                                    <div className="col-12 col-md-4">
                                        <MyLabel htmlFor="title" label={Str.subAxisTitle} isRequired={true}/>
