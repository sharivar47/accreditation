...
const PeriodList = () => {
    ...
    const result = await Api.patch(ApiUrls.changeStatusAccreditationPeriod(item.guid))
        if (result.success) {
            getCredibilityPeriod()
    ...
    const columns = [
        ...
        {label: Str.quickAccessSanjeh, value: row => <Link className="text-decoration-none" to={'/'}><i className="fa fa-gauge font-icon color-pink"/></Link>, display: ""},
        {label: Str.mehvars, value: row => <Link className="text-decoration-none" to={Address.getAxises(ApiUrls.query({accreditationId: row.guid}))}><i className="fa fa-door-open font-icon color-orange"/></Link>},
                ...
                <Card>
                    <CardHeader>
                        <div className="d-flex justify-content-between card-header-custom"><span
                            ...
                            <span className="btn-add btn d-inline-flex"><Link to={Address.createAccreditationPeriod()} className="text-decoration-none create-link">{Str.addAccreditationPeriod}</Link><i className="fa fa-plus px-1 add-icon"/></span>
                        </div>
                    </CardHeader>
                    <CardBody>
                        <Grid
