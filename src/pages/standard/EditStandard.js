...
const EditAxis = () => {
    ...
    const validationObj = Yup.object().shape({
        ...
        // shortTitle: Yup.string().required(Str.requiredField),
        code: Yup.string().required(Str.requiredField),
    ...
    const getStandard = async () => {
        const result = await Api.send(ApiUrls.getStandard(id))
        if (result.success) {
            setInit(result.data)
        }
    }
    ...
    const save = async (model) => {
        model.shortTitle = ""
        delete model.guid
        const result = await Api.put(ApiUrls.editStandard(id), model)
        if (result.success) {
        ...
        <Card>
            <CardHeader>
                <div className="d-flex justify-content-between card-header-custom bg-primary text-white"><span
                    className="text-header">{Str.editStandard}</span>
                            ...
                            return (<form className="form-wrapper" style={{paddingTop: "60px"}} onSubmit={(e) => {
                                ...
                                <div className="row mb-4">
                                    <div className="col-12">
                                        <MyLabel htmlFor="title" label={Str.standardTitle} isRequired={true}/>
                                        <MyTextArea name="title" formik={props}/>
                                        <MyErrorForm errorFor="title" formik={props}/>
                                    </div>
                                </div>
                                <div className="row mb-4">
                                    <div className="col-12 col-md-4">
                                        <MyLabel htmlFor="code" label={Str.standardCode} isRequired={true}/>
                                        <MyTextInput name="code" formik={props}/>
                                        <MyErrorForm errorFor="code" formik={props}/>
                                    </div>
                                    <div className="col-12 col-md-4">
